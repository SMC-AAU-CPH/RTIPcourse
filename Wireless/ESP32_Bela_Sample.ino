...
const IPAddress outIp(192,168,30,1);         //default IP, will change with received udp
const unsigned int outPort = 666;          // remote port to receive OSC
const unsigned int localPort = 8888;        // local port to listen for OSC packets (actually not used for sending)
const char *ssid = "MyBelaAccessPoint";             //BS SSID
...
void setup() {
    ...
    WiFi.mode(WIFI_STA);
//    WiFi.hostname("espboard");      // DHCP Hostname (useful for finding device for static lease)
    IPAddress staticip(192, 168, 30, 10);
...
void loop() {
    hallSensor = hallRead();
    OSCMessage msg("/esp"); //OSC message compilation
    msg.add(hallSensor);
